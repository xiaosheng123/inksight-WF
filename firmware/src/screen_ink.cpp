...
#include "font.h"
#include "u8g2_font_PuHuiTi14_gb.h"
#include "u8g2_font_PuHuiTi12_gb.h"
#define ROTATION 0

// 适配黑白色
// #if GxEPD2_DISPLAY_CLASS != GxEPD2_BW
#define LIGHT_CLR   GxEPD_BLACK
// #else
// #define LIGHT_CLR   GxEPD_BLACK
// #endif

GxEPD2_DISPLAY_CLASS<GxEPD2_420c_CZ15, GxEPD2_420c_CZ15::HEIGHT> display(GxEPD2_420c_CZ15(/*CS=GPIO7*/7, /*DC=GPIO1*/1, /*RST=GPIO2*/2, /*BUSY=GPIO10*/10));
U8G2_FOR_ADAFRUIT_GFX u8g2Fonts;


#define FONT_TEXT u8g2_font_PuHuiTi_14_gb // 224825bytes，最大字库（天气描述中“霾”，只有此字库中有）
#define FONT_SUB u8g2_font_PuHuiTi_12_gb // 次要字体，u8g2最小字体
...
void draw_cal_days(bool partial) {
    ...
    for (size_t iDay = 0; iDay < totalDays; iDay++) {
        ...
        if ((iDay + 1) == tmInfo.tm_mday) { // 双线加粗
            ...
            // 加框线
            display.drawRoundRect(x, y + 1, 56, 44, 4, LIGHT_CLR);
            display.drawRoundRect(x + 1, y + 2, 54, 42, 3, LIGHT_CLR);
        ...
        if (tagChar != NULL) {
            u8g2Fonts.setBackgroundColor(GxEPD_WHITE);
            u8g2Fonts.setForegroundColor(LIGHT_CLR);
